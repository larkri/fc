...
<<<<<<< HEAD
    logging.info("Block tillagd i blockchain.json.")
...
<<<<<<< HEAD
            if saved_match != match_to_list(latest_match):
                ...
                logging.info("NY MATCH HAR SPELATS OCH LAGTS TILL I MATCHES.CSV & BLOCKCHAIN.JSON:")
                logging.info(f"{latest_match['start_time']} - {latest_match['white']} vs {latest_match['black']}, Result: {latest_match['result']}")
            else:
                logging.info("Ingen ny match än...")
        else:
            logging.error("Kunde inte hämta matchdata.")
=======
